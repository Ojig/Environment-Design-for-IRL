...
def maze_maximin_environment_design(env, m_reward, s_reward, delta=0.0001):
    ...
    while True:
        ...
        for next_state in sample(range(env.state_space.n), env.state_space.n):
            ...
            neighbours = np.unique(np.array([next_state - 1, next_state + 1, next_state - (env.width + 1), next_state + (env.width + 1)]).clip(0, env.state_space.n - 1))
            for env_action in sample(range(n_env_actions), n_env_actions):
                ...
                elif env_action == 1 and current_env.grid.get(*next_state_coord) is None and env.agent_start_pos != next_state_coord:
                ...
                for neigh in neighbours:
                    for reward_index in range(n_rewards):
                        ...
                        for action in range(env.action_space.n):
                            temp_Q_rew[action] = env.gamma * np.dot(current_env.get_transition_probabilities(neigh, action), V[reward_index, :])
                        Q_env[next_state, env_action] += max(temp_Q_rew) / n_rewards
                    # mean reward function
                    temp_Q_rew = np.zeros(env.action_space.n)
                    for action in range(env.action_space.n):
                        temp_Q_rew[action] = env.gamma * np.dot(current_env.get_transition_probabilities(neigh, action), V_mean[:])
                    Q_env[next_state, env_action] -= max(temp_Q_rew)  # WE CAN ADD A FACTOR < 1 HERE TO PROMOTE SOLVABLE ENVIRONMENTS
            ...
            for reward_index in range(n_rewards):
                for s in neighbours:
                    for a in range(env.action_space.n):
                        Q[reward_index, s, a] = s_reward[reward_index][s] + env.gamma * np.dot(current_env.get_transition_probabilities(s, a), V[reward_index, :])
                    V[reward_index, s] = max(Q[reward_index, s, :])
            for s in neighbours:
                for a in range(env.action_space.n):
                    Q_mean[s, a] = m_reward[s] + env.gamma * np.dot(current_env.get_transition_probabilities(s, a), V_mean[:])
        ...
        if iterations % 10 == 0:
            ...
            print("Current Iteration Walls: Regret", evaluate_regret_of_maze(env, walls, s_reward, m_reward))
        if iterations > 100:
            break
    return walls
...
def non_blocking_domain_randomisation(env, prob=0.2):
    # probability of a wall in a before empty cell
    walls = []
    eff_state_space = [i for i in range(env.state_space.n) if (i % env.width != 0)
                       and (i % env.width != env.width - 1)
                       and (i > env.width)
                       and (i < env.width * (env.width - 1))
                       and (i != env.width + 2) and (i != env.width * 2 + 1)
                       and (i != 40) and (i != 41) and (i != 42)]    # area around starting position is wall-free # 11, 19
...
def get_environment_candidates(env, size):
    ...
    candidates.append([[3, 4], [3, 5], [3, 6], [2, 5], [3, 7], [6, 1], [5, 2], [6, 3], [5, 3], [7, 3]])
...
def evaluate_regret_of_maze(env, walls, s_reward, m_reward):
    regret = 0
    maze = ConstructedMazeEnv(size=env.width, walls=walls) # just change the transition function, maybe
...
def brute_force_maze_design(env, s_reward, m_reward, candidate_size):
    ...
    bayes_regret = []
    for idx, walls in enumerate(candidate_walls):
        bayes_regret.append(evaluate_regret_of_maze(env, walls, s_reward, m_reward))
        if idx % 10 == 0:
            print("Maze Design: Candidates No.", idx)
...
def regret_based_maze_design(env, m_reward, s_reward, delta=0.0001):
    ...
    while True:
        ...
        for state in range(env.state_space.n):
            if state < env.width or state >= env.state_space.n - env.width or state % env.width == 0 or state + 1 % env.width == 0:
                continue
            for env_action in sample(range(n_env_actions), n_env_actions):
                # get current environment
                current_walls = walls.copy()
                ...
                if env_action == 0:
                    ...
                    if current_env.grid.get(*right_coord) is None and env.agent_start_pos != right_coord:
                        current_walls.append(right_coord)
                    if bot_coord in current_walls:
                        current_walls.remove(bot_coord)
                elif env_action == 2:
                    if current_env.grid.get(*bot_coord) is None and env.agent_start_pos != bot_coord:
                ...
                for reward_index in range(n_rewards):
                    temp_Q_rew = np.zeros(env.action_space.n)
                    for action in range(env.action_space.n):
                        temp_Q_rew[action] = env.gamma * np.dot(current_env.get_transition_probabilities(state, action), V[reward_index, :])
                    Q_env[state, env_action] += max(temp_Q_rew) / n_rewards
                # mean reward function
                temp_Q_rew = np.zeros(env.action_space.n)
                for action in range(env.action_space.n):
                    temp_Q_rew[action] = env.gamma * np.dot(current_env.get_transition_probabilities(state, action), V_mean[:])
                Q_env[state, env_action] -= max(temp_Q_rew)  # WE CAN ADD A FACTOR < 1 HERE TO PROMOTE SOLVABLE ENVIRONMENTS
            ...
            for reward_index in range(n_rewards):
                for a in range(env.action_space.n):
                    Q[reward_index, state, a] = s_reward[reward_index][state] + env.gamma * np.dot(current_env.get_transition_probabilities(state, a), V[reward_index, :])
                V[reward_index, state] = max(Q[reward_index, state, :])
            # mean reward function
            for a in range(env.action_space.n):
                Q_mean[state, a] = m_reward[state] + env.gamma * np.dot(current_env.get_transition_probabilities(state, a), V_mean[:])
        ...
        if curr_regret > max_regret:
            ...
            max_walls = walls
        print("Current Iteration", iterations, "Walls: Regret", curr_regret, " ---- Max Regret:", max_regret, "Max Walls", max_walls)
    return max_walls
