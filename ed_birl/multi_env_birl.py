...
def bayesian_reward_learning(base_env, observations, sample_size, proposal_distr='grid', last_reward=None, beta=20):
    ...
    for k in range(sample_size):
        if proposal_distr == 'grid':
            proposed_reward, pdf_proposal = grid_proposal(env, last_reward, step_size=step_size)
        elif proposal_distr == 'simplex':
            proposed_reward, pdf_proposal = simplex_proposal(last_reward, n_steps=n_steps)

        likelihood, n_scales = get_likelihood(proposed_reward, compact_obs, beta=beta)
        if old_scales != n_scales:
            scale = (old_scales - n_scales) * 1e+20
        else:
            scale = 1
        p = likelihood * scale
        ...
        if k % 100 == 0:
            print("Step", k, "; Acceptance rate", round(100 * acceptance_rate, 1), "%", "; step_size", step_size, "n_steps", n_steps)
    posterior_mean = sum(posterior_samples[int(sample_size / 4):]) / (sample_size-int(sample_size / 4))
...
def grid_proposal(env, last_reward, step_size):
    proposed_reward = last_reward.copy()
    step = np.random.choice([-step_size, 0, step_size], env.state_space.n, p=(0.15, 0.7, 0.15))
