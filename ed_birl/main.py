...
for episode in range(n_episodes):
    if episode == 0:
        ...
        plot_heatmaps([0.5 for _ in range(size**2)], [1 for _ in range(size**2)], episode=episode)
        # walls = []
        walls = domain_randomisation(base_env)  # domain randomisation
    else:
        posterior_samples, posterior_mean, posterior_map, posterior_std = bayesian_reward_learning(base_env, observations, birl_sample_size, proposal_distr='grid')
        ...
        candidate_walls, bayes_regret = brute_force_maze_design(base_env, s_reward, m_reward, n_environment_samples)
