...
def plot_heatmaps(posterior_mean, posterior_std, episode=0):
    ...
    ax = sns.heatmap(split_rewards, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_posterior_mean.png', bbox_inches='tight')
    plt.close()

    # plot posterior std
    split_std = np.array_split(posterior_std, size)
    ax = sns.heatmap(split_std, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_posterior_std.png', bbox_inches='tight')
    plt.close()
    ...
    ax = sns.heatmap(split_rewards, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_rounded_mean.png', bbox_inches='tight')
    plt.close()
    ...
    ax = sns.heatmap(split_std, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_rounded_std.png', bbox_inches='tight')
    plt.close()

    # map posterior mean to (0,1)
    if max(posterior_mean) != min(posterior_mean):
        posterior_mean = [(item - min(posterior_mean)) / (max(posterior_mean) - min(posterior_mean)) for item in posterior_mean]

    # map posterior_std to (0,1)
    if max(posterior_std) != min(posterior_std):
        posterior_std = [(item - min(posterior_std)) / (max(posterior_std) - min(posterior_std)) for item in list(posterior_std)]

    # plot rounded reward heatmap
    posterior_mean = np.round(posterior_mean, decimals=1)
    split_rewards = np.array_split(np.array(posterior_mean), size)
    ax = sns.heatmap(split_rewards, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_rounded_scaled_mean.png', bbox_inches='tight')
    plt.close()
    ...
    split_std = np.array_split(posterior_std, size)
    ax = sns.heatmap(split_std, annot=True, square=True, xticklabels=False, yticklabels=False, cbar=False)
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_rounded_scaled_std.png', bbox_inches='tight')
    plt.close()
...
def save_initial_render(env, episode):
    ...
    plt.savefig('plots' + folder_no + '/' + str(episode) + '_env.png', bbox_inches='tight')
    plt.close()
